...
# --- Model Yükleme (GÜNCELLENDİ) ---
model = SurrogatePINN()
try:
    # 1. Pth dosyasını belleğe al
    state_dict = torch.load("kuantum_beyni.pth", map_location="cpu", weights_only=True)
    
    # 2. PyTorch Compile (_orig_mod.) önekini temizle
    clean_state_dict = {}
    ...
    print("\nModel ağırlıkları uyuşmuyor. Mimari farkı olabilir.")
    sys.exit(1)
...
ax1.axhline(E_final, color="red", linestyle="--", linewidth=2, label=f"E (PINN) = {E_final:.3f} eV")
ax1.axhline(E_analytic_inf, color="orange", linestyle=":", linewidth=1.5, label=f"E (∞ kuyu ref) = {E_analytic_inf:.3f} eV")
