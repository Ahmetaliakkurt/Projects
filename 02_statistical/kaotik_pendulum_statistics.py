...
# ==========================================
# 2. İSTATİSTİKSEL HESAPLAMALAR (DÜZELTİLMİŞ)
# ==========================================
theta2_all = sol_y[2::4, :] 

# --- DÜZELTME: Dairesel Standart Sapma (Sınır Patlamasını Önleme) ---
# 1. Her (t) anı için sarkaçların açısal "Kütle Merkezini" buluyoruz
mean_sin = np.mean(np.sin(theta2_all), axis=0)
mean_cos = np.mean(np.cos(theta2_all), axis=0)
mean_angle = np.arctan2(mean_sin, mean_cos)

# 2. Her sarkacın kütle merkezine olan farkını bulup [-pi, pi] aralığına sarıyoruz
# (Bu işlem tüm kümeyi sınır çizgisinden uzağa, tam merkeze 0 noktasına taşır)
diff_angle = theta2_all - mean_angle
diff_wrapped = np.arctan2(np.sin(diff_angle), np.cos(diff_angle))

# Artık güvenle standart sapma alabiliriz!
std_dev_deg = np.degrees(np.std(diff_wrapped, axis=0))

# --- Shannon Entropisi Hesabı (Aynı şekilde fark açıları üzerinden yaparsak daha pürüzsüz olur) ---
bins = 36 
entropy_bits = np.zeros(frames)
for i in range(frames):
    counts, _ = np.histogram(diff_wrapped[:, i], bins=bins, range=(-np.pi, np.pi))
